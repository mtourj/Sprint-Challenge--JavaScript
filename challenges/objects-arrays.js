...
for (let i = 0; i < graduates.length; i++) {
  if (!contains(universities, graduates[i]["university"]))
...
function sort(array) {
  ...
  for (let i = 0; i < alphacode.length; i++) {
    for (let x = 0; x < array.length; x++) {
      if (array[x][0].toUpperCase() === alphacode[i]) {
        result.push(array[x]);
      }
    }
  }
...
universities = sort(universities);
...
*/
const populations = zooAnimals.map(value => value['population']);
console.log(populations);

const populationTotal = populations.reduce((acc, val) => acc + val);
